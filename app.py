"""
Análise de Desempenho e Valor de Mercado no Futebol Europeu
Interface Streamlit — Projeto de Ciência de Dados
Dataset: Kaggle / davidcariboo / player-scores v661
"""
...
with tab1:
    ...
    data_by_pos = [
        np.log1p(df_val[df_val["position_group"] == p]["latest_market_value"].values)
        for p in pos_order
    ]
    bp = ax.boxplot(
        data_by_pos,
    ...
    for patch, pos in zip(bp["boxes"], pos_order):
        patch.set_facecolor(COLORS.get(pos, "#888"))
        patch.set_alpha(0.8)
    ax.set_title("Box Plot — log(Valor de Mercado + 1)", fontsize=13, fontweight="bold")
    ...
    ax2.set_title("Valor Mediano por Posição (€M)", fontsize=13, fontweight="bold")
    ...
    with st.expander("Insight"):
        st.markdown(
            "- **Atacantes** possuem o maior valor mediano, seguidos de **Meio-Campistas**.\n"
            "- O pico de valor ocorre tipicamente entre **22 e 27 anos**, declinando após os 30.\n"
            "- A distribuição é fortemente assimétrica (cauda longa à direita) — poucos "
            "jogadores concentram valores extremos."
        )
...
with tab4:
    ...
    nac = df[nac_col].value_counts().head(20)
...
with tab5:
    ...
    st.markdown(
        "Usamos um **Gradient Boosting Regressor** para estimar o log do valor de mercado "
        "com base em atributos do jogador e estatísticas acumuladas de carreira."
    )

    if st.button("Treinar / Recarregar Modelo", type="primary"):
        st.cache_data.clear()

    with st.spinner("Aguarde — treinando modelo..."):
        try:
    ...
    m2.metric("MAE (€)", fmt_eur(results["mae_eur"]))
    m3.metric("R² CV médio (5-fold)", f"{results['cv_r2_mean']:.3f} ± {results['cv_r2_std']:.3f}")
    m4.metric("Amostras treino / teste", f"{results['n_train']} / {results['n_test']}")
    ...
    with sc3:
        ...
        sim_red = st.number_input("Cartões Vermelhos", 0, 20, 0)
    ...
    sim_df = pd.DataFrame([{
        ...
        "total_yellow": sim_yellow, "total_red": sim_red,
